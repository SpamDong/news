...
});(function(b,a){typeof exports==="object"&&typeof module!=="undefined"?module.exports=a():typeof define==="function"&&define.amd?define(a):(b=b||self,b.Mustache=a())
})(this,function(){var z=Object.prototype.toString;
var A=Array.isArray||function d(S){return z.call(S)==="[object Array]"
};
function w(S){return typeof S==="function"
}function N(S){return A(S)?"array":typeof S
}function j(S){return S.replace(/[\-\[\]{}()*+?.,\\\^$|#\s]/g,"\\$&")
}function L(T,S){return T!=null&&typeof T==="object"&&S in T
}function f(S,T){return S!=null&&typeof S!=="object"&&S.hasOwnProperty&&S.hasOwnProperty(T)
}var q=RegExp.prototype.test;
function c(T,S){return q.call(T,S)
}var u=/\S/;
function G(S){return !c(u,S)
}var s={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;","/":"&#x2F;","`":"&#x60;","=":"&#x3D;"};
function B(S){return String(S).replace(/[&<>"'`=\/]/g,function T(U){return s[U]
})
}var v=/\s*/;
var E=/\s+/;
var n=/\s*=/;
...
var T="";
var ak=0;
function aj(){if(ap&&!al){while(Y.length){delete ac[Y.pop()]
}}else{Y=[]
}ap=false;
al=false
}var af,aa,am;
function Z(aq){if(typeof aq==="string"){aq=aq.split(E,2)
}if(!A(aq)||aq.length!==2){throw new Error("Invalid tags: "+aq)
}af=new RegExp(j(aq[0])+"\\s*");
aa=new RegExp("\\s*"+j(aq[1]));
am=new RegExp("\\s*"+j("}"+aq[1]))
}Z(ab||H.tags);
var V=new a(an);
var W,U,ae,ah,X,S;
while(!V.eos()){W=V.pos;
ae=V.scanUntil(af);
if(ae){for(var ai=0,ag=ae.length;
ai<ag;
++ai){ah=ae.charAt(ai);
if(G(ah)){Y.push(ac.length);
T+=ah
}else{al=true;
ao=true;
T+=" "
}ac.push(["text",ah,W,W+1]);
W+=1;
if(ah==="\n"){aj();
T="";
ak=0;
...
U=V.scan(t)||"name";
V.scan(v);
if(U==="="){ae=V.scanUntil(n);
V.scan(n);
V.scanUntil(aa)
}else{if(U==="{"){ae=V.scanUntil(am);
V.scan(P);
V.scanUntil(aa);
U="&"
}else{ae=V.scanUntil(aa)
}}if(!V.scan(aa)){throw new Error("Unclosed tag at "+V.pos)
}if(U==">"){X=[U,ae,W,V.pos,T,ak,ao]
}else{X=[U,ae,W,V.pos]
}ak++;
ac.push(X);
if(U==="#"||U==="^"){ad.push(X)
}else{if(U==="/"){S=ad.pop();
if(!S){throw new Error('Unopened section "'+ae+'" at '+W)
}if(S[1]!==ae){throw new Error('Unclosed section "'+S[1]+'" at '+W)
}}else{if(U==="name"||U==="{"||U==="&"){al=true
}else{if(U==="="){Z(ae)
}}}}}aj();
S=ad.pop();
if(S){throw new Error('Unclosed section "'+S[1]+'" at '+V.pos)
}return o(r(ac))
}function r(X){var T=[];
var V,S;
...
U<W;
++U){V=X[U];
if(V){if(V[0]==="text"&&S&&S[0]==="text"){S[1]+=V[1];
S[3]=V[3]
}else{T.push(V);
S=V
}}}return T
...
var Y=[];
var T,V;
for(var S=0,U=X.length;
S<U;
++S){T=X[S];
switch(T[0]){case"#":case"^":W.push(T);
Y.push(T);
W=T[4]=[];
break;
case"/":V=Y.pop();
V[5]=T[2];
W=Y.length>0?Y[Y.length-1][4]:Z;
break;
default:W.push(T)
}}return Z
}function a(S){this.string=S;
this.tail=S;
this.pos=0
}a.prototype.eos=function M(){return this.tail===""
};
a.prototype.scan=function Q(U){var T=this.tail.match(U);
...
a.prototype.scanUntil=function K(U){var T=this.tail.search(U),S;
switch(T){case -1:S=this.tail;
this.tail="";
break;
case 0:S="";
break;
default:S=this.tail.substring(0,T);
this.tail=this.tail.substring(T)
}this.pos+=S.length;
return S
};
function O(T,S){this.view=T;
this.cache={".":this.view};
this.parent=S
}O.prototype.push=function I(S){return new O(S,this)
};
O.prototype.lookup=function k(W){var U=this.cache;
var Y;
if(U.hasOwnProperty(W)){Y=U[W]
}else{var X=this,T,Z,V,S=false;
while(X){if(W.indexOf(".")>0){T=X.view;
Z=W.split(".");
V=0;
while(T!=null&&V<Z.length){if(V===Z.length-1){S=L(T,Z[V])||f(T,Z[V])
}T=T[Z[V++]]
}}else{T=X.view[W];
S=L(X.view,W)
}if(S){Y=T;
...
}if(w(Y)){Y=Y.call(this.view)
}return Y
};
function m(){this.cache={}
}m.prototype.clearCache=function C(){this.cache={}
};
m.prototype.parse=function x(U,T){var S=this.cache;
var W=U+":"+(T||H.tags).join(":");
var V=S[W];
if(V==null){V=S[W]=e(U,T)
}return V
};
m.prototype.render=function D(W,S,V,T){var X=this.parse(W,T);
var U=S instanceof O?S:new O(S,undefined);
return this.renderTokens(X,U,V,W,T)
};
m.prototype.renderTokens=function p(Z,S,X,ab,ac){var V="";
var U,T,aa;
...
U=Z[W];
T=U[0];
if(T==="#"){aa=this.renderSection(U,S,X,ab)
}else{if(T==="^"){aa=this.renderInverted(U,S,X,ab)
}else{if(T===">"){aa=this.renderPartial(U,S,X,ac)
}else{if(T==="&"){aa=this.unescapedValue(U,S)
}else{if(T==="name"){aa=this.escapedValue(U,S)
}else{if(T==="text"){aa=this.rawValue(U)
}}}}}}if(aa!==undefined){V+=aa
}}return V
};
m.prototype.renderSection=function y(U,S,X,aa){var ab=this;
var W="";
var Y=S.lookup(U[1]);
function T(ac){return ab.render(ac,S,X)
}if(!Y){return
}if(A(Y)){for(var V=0,Z=Y.length;
V<Z;
++V){W+=this.renderTokens(U[4],S.push(Y[V]),X,aa)
}}else{if(typeof Y==="object"||typeof Y==="string"||typeof Y==="number"){W+=this.renderTokens(U[4],S.push(Y),X,aa)
}else{if(w(Y)){if(typeof aa!=="string"){throw new Error("Cannot use higher-order sections without the original template")
}Y=Y.call(S.view,aa.slice(U[3],U[5]),T);
if(Y!=null){W+=Y
}}else{W+=this.renderTokens(U[4],S,X,aa)
}}}return W
};
m.prototype.renderInverted=function b(U,T,S,W){var V=T.lookup(U[1]);
if(!V||A(V)&&V.length===0){return this.renderTokens(U[4],T,S,W)
}};
m.prototype.indentPartial=function l(S,U,T){var X=U.replace(/[^ \t]/g,"");
var W=S.split("\n");
for(var V=0;
V<W.length;
V++){if(W[V].length&&(V>0||!T)){W[V]=X+W[V]
}}return W.join("\n")
};
m.prototype.renderPartial=function J(U,T,V,aa){if(!V){return
}var Y=w(V)?V(U[1]):V[U[1]];
if(Y!=null){var Z=U[6];
var W=U[5];
var X=U[4];
var S=Y;
if(W==0&&X){S=this.indentPartial(Y,X,Z)
}return this.renderTokens(this.parse(S,aa),T,V,S)
}};
m.prototype.unescapedValue=function h(T,S){var U=S.lookup(T[1]);
if(U!=null){return U
}};
m.prototype.escapedValue=function F(T,S){var U=S.lookup(T[1]);
if(U!=null){return H.escape(U)
}};
m.prototype.rawValue=function g(S){return S[1]
};
var H={name:"mustache.js",version:"3.2.1",tags:["{{","}}"],clearCache:undefined,escape:undefined,parse:undefined,render:undefined,to_html:undefined,Scanner:undefined,Context:undefined,Writer:undefined};
var R=new m;
H.clearCache=function C(){return R.clearCache()
};
H.parse=function x(T,S){return R.parse(T,S)
};
H.render=function D(V,S,U,T){if(typeof V!=="string"){throw new TypeError('Invalid template! Template should be a "string" but "'+N(V)+'" was given as the first argument for mustache#render(template, view, partials)')
}return R.render(V,S,U,T)
};
H.to_html=function i(V,T,U,W){var S=H.render(V,T,U);
if(w(W)){W(S)
}else{return S
}};
H.escape=B;
